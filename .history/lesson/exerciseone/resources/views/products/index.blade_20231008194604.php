...
<div class="col-md-12 container mb-3"> <a href="{{route('products.create')}}" class="btn btn-primary btn-sm rounded-0">
    ...
    </div> <div class="col-md-12"> <table class="table table-sm table-hover border container">
    <thead>
        <tr>
        <th>No</th>
        <th>Name</th>
    ...
    <td>{{$product->price}}</td>
    <td>
        <!-- <img src="{{ asset($product->image) }}" class="rounded" style="width:50px;height:50px"
        alt="{{$product->image}}">
        ...
        <img src="{{ URL::asset('images/' . $product->image) }}" class="rounded" style="width: 50px; height: 50px"
            alt="{{ $product->image }}">
        ...
        <img src="{{ asset('storage/images/'.$product->image) }}" class="rounded" style="width: 50px; height: 50px"
            alt="{{ $product->image }}"> <img src="{{ asset('storage/images/'.$product->image) }}" class="rounded"
    ...
    <form class="formdelete" action="{{route('products.destroy',$product->id)}}" method="POST" class="d-flex
    ...
    @csrf
        ...
        @secti      $(document).re         $('.formdelete').on('su         if (confirm('Are you sure want to delete it?')) {
